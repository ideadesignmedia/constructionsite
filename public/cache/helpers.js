...
follow = (event, targetid) => {
    return new Promise((res, rej) => {
        ...
        let fall = new XMLHttpRequest()
        fall.onerror = (e) => {
            return rej(e)
        }
        fall.onreadystatechange = () => {
            if (fall.readyState === 4) {
                let data = JSON.parse(fall.responseText)
                console.log(JSON.stringify(fall.responseText))
                if (data.error === false) {
                    return res(true)
                } else {
                    return res(false)
        ...
        fall.open('post', '/idm/tracking', true)
        fall.setRequestHeader('content-type', 'application/json')
        fall.send(JSON.stringify(that))
    })
}